...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    // Load user interests from localStorage if available
  ...
  // Save user interests to localStorage when they change
  useEffect(() => {
  ...
  const trackItemView = (item) => {
    setViewedItems(prev => {
      ...
      return [...prev, item].slice(-5); // Keep only last 5 viewed items
    });
  };

  // Toggle expanded view for a service
  const toggleExpand = (categoryIndex, itemIndex, item) => {
    trackItemView(item);
  ...
  // Get recommended items based on user interests or viewed items
    ...
    <div className="min-h-screen flex flex-col relative">
      ...
      <AnimatePresence>
        {showOnboarding && (
          ...
          >
            <motion.div 
              className="glass bg-[var(--current-bg)] p-8 rounded-2xl max-w-md w-full mx-4"
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-primary">Welcome to Our Studio</h2>
                <button 
                  onClick={completeOnboarding}
                  className="text-[var(--current-text)] hover:text-primary transition-colors"
                >
                  <FiX size={24} />
                </button>
              </div>
              
              <p className="mb-6">Let us personalize your experience. What types of design are you interested in?</p>
              
              <div className="space-y-4 mb-8">
                {['Poster Designs', 'Brand Identity', 'Digital Designs', 'Print', 'Web', 'Marketing'].map((interest) => (
                  <button
                    key={interest}
                    onClick={() => toggleInterest(interest)}
                    className={`w-full text-left px-4 py-3 rounded-lg transition-all ${userInterests.includes(interest) 
              ...
              <button
                onClick={completeOnboarding}
                className="btn-gradient w-full py-3 rounded-lg font-medium text-white"
              >
                Continue to Site
              </button>
            </motion.div>
          ...
          >
            ...
            <div className="p-3 border-t border-[var(--current-nav)]">
              <div className="flex">
                ...
                <button
                  onClick={sendMessage}
                  className="bg-primary text-white px-3 py-2 rounded-r-lg hover:opacity-90 transition-opacity"
                >
                  Send
                </button>
              </div>
  ...
  >
    ...
    <div className="w-6 h-10 border-2 border-[var(--current-text)] rounded-full flex justify-center items-start">
      ...
      {recommendedItems.length > 0 && (
        <section className="py-16 bg-[var(--current-bg)]">
          <div className="max-w-7xl mx-auto px-6">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-3xl font-bold mb-4 text-gradient bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
                {userInterests.length > 0 ? 'Recommended For You' : 'Featured Work'}
              </h2>
            ...
            >
              {recommendedItems.map((item, index) => (
                <motion.div
                  key={index}
                  className="card-3d glass bg-[var(--current-nav)] rounded-xl overflow-hidden shadow-lg"
                  variants={item}
          ...
          {serviceCategory.items.slice(0, visibleItems).map((item, itemIndex) => (
            ...
            >
              ...
              <div className="p-6 text-white">
                <h4 className="text-xl font-bold mb-2 text-white drop-shadow">
                  {item.title}
                </h4>
                <p className="opacity-80 mb-4 text-sm">{item.description}</p>
                ...
                <div className="flex justify-between items-center">
                  <a
                    href={item.link}
                    className="inline-flex items-center text-primary hover:underline font-medium transition-all"
                  >
                    View examples <FiArrowRight className="ml-2" />
                  </a>

                  <button
                    onClick={() => toggleExpand(categoryIndex, itemIndex, item)}
                ...
                <div className="p-6 border-t border-white/10 bg-[var(--current-nav)]/60 backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {item.images.slice(1).map((image, imgIndex) => (
                      <div
                        key={imgIndex}
                        className="h-32 overflow-hidden rounded-lg shadow-inner"
                      >
                        <img
                          src={image}
